...
class CMMypageMainTable: UIView {
	var table:UITableView?
	var tablehead:UIImageView?
	...
	public func setTableHeadPicture(image:UIImage){
		tablehead?.image = image
	}
	private func createTableHeadView(){
		
		tablehead = UIImageView()
		tablehead?.isUserInteractionEnabled = true
		tablehead?.kf.setImage(with: URL(string: "http://pic.58pic.com/00/94/40/47bOOOPIC5f.jpg"))
...
extension CMMypageMainTable : UITableViewDelegate,UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		let cell:CMMypageMainTableCell =  CMMypageMainTableCell()
		return cell
	}
	...
	func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
		...
		print(tabH)
		return 2.0 * Constant.screenHeight / 3.0 - tabH
	}
	
	func scrollViewWillBeginDragging(_ scrollView: UIScrollView) {
 		findMemberSubView(superview: scrollView)
		
	}
	
	private func findMemberSubView(superview:UIView){
		if superview.subviews.count != 0 {
			for vws in superview.subviews {
 				if vws.isMember(of: UITextField.self){
  					vws.resignFirstResponder()
 				}
				findMemberSubView(superview: vws)
			}
 		}
